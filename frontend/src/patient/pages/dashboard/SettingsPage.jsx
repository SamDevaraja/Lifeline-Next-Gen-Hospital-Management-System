...
import {
    Activity, ShieldCheck, CheckCircle, User, Lock, Mail, Smartphone, MapPin, ChevronDown, Globe, RefreshCw
} from 'lucide-react';
...
const SettingsPage = ({ user, onUpdate }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2 max-w-6xl mx-auto px-4 pt-1 pb-4 font-sans">
                ...
                {showSuccessModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[var(--luna-card)] rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl border border-[var(--luna-border)] text-center overflow-hidden">
                            ...
                            <h2 className="text-2xl font-bold mb-2 tracking-tight" style={{ color: 'var(--luna-text-main)' }}>Credentials Updated</h2>
                            <p className="text-[11px] font-bold leading-relaxed opacity-40 mb-8 px-4" style={{ color: 'var(--luna-text-main)' }}>
                                Your security protocol has been successfully re-synchronized with the institutional registry.
                            </p>
            ...
            <div className="max-w-3xl mx-auto w-full flex flex-col items-center text-center pb-4 border-b border-[var(--luna-border)] mb-6">
                <h1 className="text-xl font-bold tracking-tight" style={{ color: 'var(--luna-text-main)' }}>Profile Configuration</h1>
            ...
            <div className="w-full max-w-3xl mx-auto">
                        ...
                        <form onSubmit={handleUpdate} className="space-y-2.5">
                            ...
                            {(isDoctor || isPatient) && (
                                <div className="space-y-1.5 md:col-span-2">
                                    <label className="text-[9px] font-bold uppercase tracking-widest text-slate-500 px-1 opacity-85">
                                    ...
                                    <textarea
                                        ...
                                        className="input !py-3.5 !px-4 w-full min-h-[120px] focus:bg-white/5"
                                        placeholder={isDoctor ? 'Describe your medical specializations...' : 'Document your current physiological status...'}
                                    />
                                </div>
                            )}
                ...
                <div className="text-center py-6">
                    <p className="text-[10px] font-bold uppercase tracking-widest opacity-20">Settings Management System V2.0</p>
                </div>
            </div>
        </motion.div>
    );
