...
const Signup = () => {
    ...
    const [form, setForm] = useState({ firstName: '', lastName: '', email: '', username: '', mobile: '', address: '', password: '', confirm: '', department: DEPARTMENTS[0], symptoms: '' });
        ...
        onSuccess: async (tokenResponse) => {
            ...
            try {
                ...
                if (userRes.data.role !== 'patient') {
                    localStorage.clear();
                    toast.error('Access Denied. Internal staff must use the secure Clinical portal.', { id: loadingId });
                    return;
                }
        ...
        <div className="min-h-screen pt-20 md:pt-32 pb-10 md:pb-20 flex items-start justify-center px-4 md:px-6" style={{ background: 'var(--luna-bg)' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
                ...
                <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input id="signup-symptoms" type="text" value={form.symptoms} onChange={update('symptoms')}
                                className="input !pl-14" placeholder="Summarize health concerns for initial screening" />
                        </div>
                    </div>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                        ...
                        <span className="text-sm font-bold" style={{ color: 'var(--luna-text-main)', opacity: 0.8 }}>
                            I agree to the <Link to="/patient/terms-of-service" onClick={e => e.stopPropagation()} className="font-black hover:underline" style={{ color: LUNA.teal }}>Terms of Service</Link> and{' '}
                            <Link to="/patient/privacy-policy" onClick={e => e.stopPropagation()} className="font-black hover:underline" style={{ color: LUNA.teal }}>Privacy Policy</Link>
                        </span>
                    </label>
                ...
                <p className="mt-8 text-center text-sm font-bold" style={{ color: 'var(--luna-text-main)', opacity: 0.8 }}>
                    Already registered?{' '}
                    <Link to="/patient/login" className="font-black hover:underline" style={{ color: LUNA.teal }}>Sign In here</Link>
                </p>
            </motion.div>
        </div>
    );
