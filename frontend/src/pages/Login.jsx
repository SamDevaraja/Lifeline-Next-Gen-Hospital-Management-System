    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (userRes.data.role === 'patient') {
                localStorage.clear();
                toast.error('Access Denied. This terminal is restricted to Clinical Staff. Please log in via the designated Patient Portal.');
                setLoading(false);
                return;
            }
            ...
            sessionStorage.setItem('login-toast', `Secure clinical session established for ${userRes.data.first_name || userRes.data.username}.`);
            ...
            <div className="flex-1 flex items-center justify-center px-6 py-16 relative z-10">
                <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
                    ...
                    <div className="mb-8 text-center">
                        <h1 className="text-3xl font-black mb-2 tracking-tight" style={{ color: 'var(--luna-text-main)' }}>Staff Terminal</h1>
                        <p className="font-bold text-[15px]" style={{ color: 'var(--luna-text-main)', opacity: 0.8 }}>Authorized personnel only. Please verify identity.</p>
                    </div>
